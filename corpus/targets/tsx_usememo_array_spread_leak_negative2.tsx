...
export default function SearchResults({ query }: { query: string }) {
  ...
  // SAFE: useRef provides a truly stable reference for constant data
  const baseItems = useRef(['apple', 'banana', 'cherry'])

  const filtered = useMemo(
    () => baseItems.current.filter((item) => item.includes(query)),
    [query]
  )
