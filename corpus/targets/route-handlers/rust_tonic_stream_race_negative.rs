...
impl CounterService {
    pub async fn handle_stream(&self, mut stream: Streaming<CounterRequest>) -> Result<Response<ReceiverStream<Result<CounterResponse, Status>>>, Status> {
        // SAFE: Per-stream state avoids shared mutable state across concurrent handlers.
        let per_stream_counts: Vec<u64> = Vec::new();
        let stream_counts = Arc::new(Mutex::new(per_stream_counts));
        let (tx, rx) = mpsc::channel(1024);
        tokio::spawn(async move {
            while let Some(req) = stream.message().await.unwrap_or(None) {
                let mut guard = stream_counts.lock().await;
