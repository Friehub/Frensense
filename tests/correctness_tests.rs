...
fn test_symbol_shadowing() {
    ...
    let symbols = auditor.discover_symbols(path, content, &lang, &tree).unwrap();
...
fn test_taint_through_destructuring() {
    ...
    let analyzer = gensense::semantics::data_flow::DataFlowAnalyzer::new(&context, tree.root_node());
    ...
    impl gensense::GenSenseRule for DummyRule {
        fn metadata(&self) -> &gensense::RuleMetadata { &self.metadata }
        fn check<'a>(&self, _n: tree_sitter::Node<'a>, _c: &GenSenseContext<'a>) -> Vec<gensense::Advisory> { vec![] }
        fn applies_to(&self, _ext: &str) -> bool { true }
    }
    ...
    let advisories = analyzer.analyze_block(tree.root_node(), &source_re, &sink_re, &rule, taint_reg);
    assert!(!advisories.is_empty(), "Taint should flow through destructuring to sink(a)");
}
