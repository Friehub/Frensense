...
"#;
    ...
    assert!(has_custom, "Custom YAML rule should have been loaded and fired. Found: {:?}", advisories);
}
...
"#;
    ...
    assert!(!has_panic, "Violation should have been suppressed by .gensense-suppress.yml");
}
...
"#;
    ...
    let panic_adv = advisories.iter().find(|a| a.rule_id == "RUST_PANIC_IN_LIB").expect("Rule should fire");
    assert_eq!(panic_adv.severity, gensense::Severity::Critical, "Severity should have been overridden to Critical");
}
