...
fn run_test(rule_id: &str, content: &str, expected_count: usize, ext: &str) {
    ...
    assert_eq!(rule_findings.len(), expected_count, 
        "Rule {} expected {} findings but got {}. Content: {}", 
        rule_id, expected_count, rule_findings.len(), content);
}
...
fn test_rust_blocking_io() {
    ...
    run_test(rule_id, "async fn t() { tokio::time::sleep(d).await; }", 0, "rs");
}
...
fn test_rust_clone_in_loop() {
    ...
    // Positive
    run_test(rule_id, "fn t() { for x in v { let y = x.clone(); } }", 1, "rs");
    
    // Negative
    run_test(rule_id, "fn t() { let y = x.clone(); for x in v { } }", 0, "rs");
}
...
fn test_ts_god_function() {
    ...
    for i in 0..100 {
        big_func.push_str(&format!("  console.log({});\n", i));
    }
    big_func.push('}');
