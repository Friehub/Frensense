...
use crate::spec::{
    call_last_segment, Import, LanguageSpec, NodeRole,
...
fn classify_rust(kind: &str) -> NodeRole {
    match kind {
        ...
        | "while_let_expression" | "loop_expression" => NodeRole::Loop,
        ...
        | "field_identifier" | "primitive_type" => NodeRole::Identifier,
        "string_literal" | "raw_string_literal"
        | "integer_literal" | "float_literal" | "boolean_literal" => NodeRole::Literal,
...
fn rust_package_category(pkg: &str) -> Option<PackageCategory> {
    match pkg {
        "axum" | "actix-web" | "actix_web" | "rocket"
        | "warp" | "tide" | "poem" | "salvo"
        | "ntex" | "viz" | "gotham" => Some(PackageCategory::HttpFramework),

        "sqlx" | "diesel" | "sea-orm" | "sea_orm"
        | "tokio-postgres" | "rusqlite" | "mysql"
        ...
        | "isahc" | "attohttpc" => Some(PackageCategory::HttpClient),

        "tera" | "handlebars" | "askama" | "minijinja" | "liquid" => {
            Some(PackageCategory::TemplateEngine)
        }

        "serde_pickle" | "bincode" | "rmp-serde"
        | "ciborium" | "postcard" => Some(PackageCategory::Deserialization),

        "ring" | "rustls" | "openssl" | "aes"
        | "sha2" | "hmac" | "rand" => Some(PackageCategory::Crypto),
...
fn rust_classify_param(_name: Option<&str>, ann: Option<&str>) -> Option<TaintOrigin> {
    ...
    match a {
        // Axum extractors
        "Json" | "Form" | "Query" | "Path" | "Bytes"
        | "Multipart" | "TypedHeader" | "Extension"
        | "RawBody" | "RawQuery" | "RawPath" => Some(TaintOrigin::UserInput),
        // Actix-web extractors
        "web::Json" | "web::Form" | "web::Query"
        | "web::Path" | "web::Bytes" | "web::Payload"
...
static RUST_PROPAGATORS: &[PropagatorRule] = &[
    ...
    PropagatorRule { call: "format_args", tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "write",       tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "writeln",     tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "println",     tainted_arg: None,    tainted_receiver: false },

    // String conversions — receiver taints return
    PropagatorRule { call: "to_string",   tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "to_owned",    tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "clone",       tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "into",        tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "as_str",      tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "as_bytes",    tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "as_ref",      tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "from_utf8",   tainted_arg: Some(0), tainted_receiver: false },
    ...
    PropagatorRule { call: "to_lowercase",tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "to_uppercase",tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "trim",        tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "trim_start",  tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "trim_end",    tainted_arg: None,    tainted_receiver: true  },
    ...
    PropagatorRule { call: "concat",      tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "push_str",    tainted_arg: Some(0), tainted_receiver: true  },
    PropagatorRule { call: "push",        tainted_arg: Some(0), tainted_receiver: true  },
    ...
    PropagatorRule { call: "collect",     tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "flat_map",    tainted_arg: None,    tainted_receiver: true  },

    // unwrap / expect: propagates taint from Result<T,_> / Option<T>
    PropagatorRule { call: "unwrap",      tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "expect",      tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "unwrap_or",   tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "ok",          tainted_arg: None,    tainted_receiver: true  },
    PropagatorRule { call: "ok_or",       tainted_arg: None,    tainted_receiver: true  },
];
...
impl LanguageSpec for RustSpec {
    ...
    fn classify(&self, kind: &str) -> NodeRole { classify_rust(kind) }

    fn wrap_region(&self, code: &str) -> String {
        format!("fn _region() {{\n{}\n}}", code)
    }

    fn import_node_kinds(&self) -> &'static [&'static str] { &["use_declaration"] }

    fn extract_imports<'tree>(&self, root: Node<'tree>, source: &str) -> Vec<Import> {
        extract_rust_imports(root, source)
    }

    fn symbol_query(&self) -> Option<&'static str> { Some(RUST_SYMBOL_QUERY) }
    fn call_query(&self)   -> Option<&'static str> { Some(RUST_CALL_QUERY) }
    ...
    fn is_http_route_decorator(&self, name: &str) -> bool {
        ...
        matches!(name, "get" | "post" | "put" | "delete" | "patch" | "options" | "head" | "route")
    }
    ...
    fn known_source_patterns(&self) -> &'static [&'static str] {
        &[
            // Axum
            "Json", "Path", "Query", "Form", "Bytes", "Multipart",
            // Actix
    ...
    fn propagator_rules(&self) -> &'static [PropagatorRule] { RUST_PROPAGATORS }
    ...
    fn route_context_hints(&self) -> &'static [&'static str] {
        &[
            "#[get(", "#[post(", "#[put(", "#[delete(", "#[patch(",  // Rocket
            ".route(", ".get(", ".post(",                              // Axum/Warp
            "IntoResponse", "impl Responder", "HttpResponse",
            "Json(", "StatusCode::",
            "web::scope", "web::resource",                            // Actix
        ]
    }

    fn test_context_hints(&self) -> &'static [&'static str] {
        &["#[test]", "#[tokio::test]", "#[async_std::test]", "assert_eq!", "assert!", "proptest!"]
    }

    fn response_method_names(&self) -> &'static [&'static str] {
        &[
            "ok", "created", "internal_server_error", "into_response",
            "content_type", "body", "finish",
        ]
    }
