...
use crate::spec::{
    call_last_segment, node_text, Import, LanguageSpec, NodeRole, PackageCategory,
...
fn js_package_category(pkg: &str) -> Option<PackageCategory> {
    ...
    match base {
        // HTTP frameworks
        "express" | "fastify" | "koa" | "hapi" | "@hono" | "hono"
        | "polka" | "h3" | "elysia" | "next" | "nuxt"
        | "@nestjs" | "nest" | "@adonisjs" => Some(PackageCategory::HttpFramework),

        // SQL
        "pg" | "postgres" | "mysql" | "mysql2" | "mariadb"
        | "sqlite3" | "better-sqlite3" | "mssql" | "oracledb"
        | "sequelize" | "knex" | "typeorm" | "@prisma"
        | "prisma" | "slonik" | "drizzle-orm" => Some(PackageCategory::SqlDatabase),

        // NoSQL
        "mongodb" | "mongoose" | "redis" | "ioredis"
        | "cassandra-driver" | "couchdb" | "@elastic" => Some(PackageCategory::NoSqlDatabase),

        // Command execution
        "child_process" | "shelljs" | "execa"
        | "cross-spawn" | "node-pty" | "spawn-command" => Some(PackageCategory::CommandExecution),

        // HTTP clients (SSRF)
        "node-fetch" | "axios" | "got" | "superagent"
        | "undici" | "request" | "node:http" | "node:https" => Some(PackageCategory::HttpClient),

        // File system (path traversal)
        "fs" | "node:fs" | "fs-extra" | "graceful-fs"
        | "recursive-readdir" | "glob" | "rimraf" => Some(PackageCategory::FileSystem),

        // Template engines (SSTI / XSS)
        "ejs" | "pug" | "handlebars" | "nunjucks"
        | "mustache" | "dot" | "art-template" | "consolidate" => {
            Some(PackageCategory::TemplateEngine)
        }

        // Unsafe deserialization
        "node-serialize" | "serialize-javascript"
        | "js-yaml" | "yaml" => Some(PackageCategory::Deserialization),
...
fn js_classify_sanitizer(call: &str) -> Option<SanitizerKind> {
    match call_last_segment(call) {
        // HTML escape
        "escape" | "escapeHtml" | "escapeHTML" | "encodeHTML"
        | "sanitizeHtml" | "sanitize" | "clean" => Some(SanitizerKind::HtmlEscape),

        // URL encode
        "encodeURIComponent" | "encodeURI" | "encode" => {
            Some(SanitizerKind::UrlEncode)
        }

        // Numeric coercion — input is definitely a number after this
        "parseInt" | "parseFloat" | "Number" | "BigInt"
        | "toFixed" | "toPrecision" => Some(SanitizerKind::Full),

        // SQL parameterization (knex, sequelize, pg style)
        "sqlEscape" | "escapeId" | "format" | "literal" | "raw" => Some(SanitizerKind::SqlParameterize),
...
static JS_PROPAGATORS: &[PropagatorRule] = &[
    ...
    PropagatorRule { call: "concat",    tainted_arg: None,    tainted_receiver: true },
    ...
    PropagatorRule { call: "slice",     tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "substring", tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "trim",      tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "trimStart", tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "trimEnd",   tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "toLowerCase",tainted_arg: None,   tainted_receiver: true },
    PropagatorRule { call: "toUpperCase",tainted_arg: None,   tainted_receiver: true },
    PropagatorRule { call: "split",     tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "toString",  tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "padStart",  tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "padEnd",    tainted_arg: None,    tainted_receiver: true },

    // Array methods — receiver taints return
    PropagatorRule { call: "map",       tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "filter",    tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "flatMap",   tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "reduce",    tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "flat",      tainted_arg: None,    tainted_receiver: true },

    // JSON
    PropagatorRule { call: "parse",     tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "stringify", tainted_arg: Some(0), tainted_receiver: false },

    // Buffer / encoding
    PropagatorRule { call: "from",      tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "toString",  tainted_arg: None,    tainted_receiver: true },
    PropagatorRule { call: "atob",      tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "btoa",      tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "decodeURIComponent", tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "encodeURIComponent", tainted_arg: Some(0), tainted_receiver: false },

    // Template tags
    PropagatorRule { call: "format",    tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "template",  tainted_arg: Some(0), tainted_receiver: false },
    PropagatorRule { call: "render",    tainted_arg: Some(0), tainted_receiver: false },

    // Path manipulation
    PropagatorRule { call: "join",      tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "resolve",   tainted_arg: None,    tainted_receiver: false },
    PropagatorRule { call: "normalize", tainted_arg: Some(0), tainted_receiver: false },
];
...
impl LanguageSpec for TypeScriptSpec {
    ...
    fn extensions(&self) -> &'static [&'static str] { &["ts", "tsx", "mts", "cts"] }
    ...
    fn classify(&self, kind: &str) -> NodeRole { classify_js(kind) }
    ...
    fn is_http_route_decorator(&self, decorator_name: &str) -> bool {
        matches!(
            ...
            | "Controller" | "Route" | "HttpGet" | "HttpPost"
    ...
    fn propagator_rules(&self) -> &'static [PropagatorRule] { JS_PROPAGATORS }
    ...
    fn route_context_hints(&self) -> &'static [&'static str] {
        &[
            "(req, res)", "app.get(", "router.get(", "app.post(", "router.post(",
            "res.send", "res.json", "res.status", "c.req", "c.json",
            "ctx.body", "ctx.response", "fastify.get(", "fastify.post(",
        ]
    }

    fn test_context_hints(&self) -> &'static [&'static str] {
        &["describe(", " it(", "test(", "expect(", "jest.", "vitest.", "chai.", "assert."]
    }

    fn response_method_names(&self) -> &'static [&'static str] {
        &[
            "json", "send", "redirect", "status", "render", "end",
            "write", "setHeader", "cookie", "clearCookie", "type",
            "format", "attachment",
        ]
    }
    ...
    fn db_api_method_names(&self) -> &'static [&'static str] {
        &[
            "query", "execute", "prepare", "raw", "find", "findOne", "findMany",
            "insert", "update", "delete", "create", "save", "select", "from",
            "where", "join", "aggregate", "count", "transaction", "commit",
            "rollback", "upsert",
        ]
    }

    fn shell_api_method_names(&self) -> &'static [&'static str] {
        &["exec", "spawn", "execFile", "execSync", "spawnSync", "system"]
    }
...
impl LanguageSpec for JavaScriptSpec {
    ...
    fn import_node_kinds(&self) -> &'static [&'static str] { &["import_statement"] }
    ...
    fn call_query(&self) -> Option<&'static str>   { TypeScriptSpec.call_query() }
    ...
    fn known_sink_names(&self) -> &'static [(&'static str, &'static str)] { JS_SINK_NAMES }

    fn known_source_patterns(&self) -> &'static [&'static str] { JS_SOURCE_PATTERNS }
...
fn classify_js_param(name: Option<&str>, ann: Option<&str>) -> Option<TaintOrigin> {
    ...
    if let Some(a) = ann {
        let base = a.trim_start_matches(':').trim().split(['<', '[', ' ']).next().unwrap_or(a);
        let base = base.rsplit('.').next().unwrap_or(base);
        if matches!(
            base,
            "Request" | "IncomingMessage" | "FastifyRequest"
            | "Context" | "HonoContext" | "KoaContext"
            | "APIGatewayProxyEvent" | "HttpRequest"
        ) {
...
static JS_SOURCE_PATTERNS: &[&str] = &[
    ...
    "c.req.raw", "c.req.query", "c.req.param",
    "c.req",
