...
const STATE_MUTATING_ROUTE_VERBS: &[&str] = &[
    ...
    "server.post", "server.put", "server.delete", "server.patch",
];

impl SemanticPattern for CsrfMissingToken {
    fn id(&self) -> &str { "CSRF_MISSING_TOKEN" }
            ...
            if node.kind() == "call_expression" {
                if let Some(func) = node.child_by_field_name("function") {
                    if let Ok(call_text) = func.utf8_text(source.as_bytes()) {
                        let is_state_mutating = STATE_MUTATING_ROUTE_VERBS.iter()
                            .any(|v| call_text.contains(v));

                        if is_state_mutating {
                            let handler_node = node.child_by_field_name("arguments")
                                .and_then(|args| {
                                    let mut c = args.walk();
                                    let mut last = None;
