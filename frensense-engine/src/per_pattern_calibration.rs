...
use crate::minhash;
use crate::pattern::scorer::{type_usage_overlap, PatternScorer};
...
pub fn train_per_pattern_calibration(
    ...
    for pattern in patterns {
        ...
        let train_pos_owned: Vec<FunctionFingerprint> = train_pos.iter().map(|f| (*f).clone()).collect();
        let val_fps: Vec<&FunctionFingerprint> = val_pos.iter().copied().chain(neg.iter()).collect();
        let labels: Vec<f64> = val_pos.iter().map(|_| 1.0).chain(neg.iter().map(|_| 0.0)).collect();
...
pub fn calibrate(raw_score: f64, params: Option<&CalibrationParams>) -> f64 {
    let Some(&(a, b)) = params else { return raw_score };
    let z = a as f64 * raw_score + b as f64;
