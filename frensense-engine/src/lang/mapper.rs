...
fn node_role_to_abstract_kind(role: NodeRole, ts_kind: &str, language: Language) -> AbstractKind {
    match role {
        // ── Definitions ──────────────────────────────────────────────────
        NodeRole::Function { is_method: true, .. }  => AbstractKind::MethodDef,
        NodeRole::Function { is_method: false, .. } => {
            // Closures and lambdas still want Closure in the abstract kind
            let is_closure = matches!(
                ts_kind,
                "arrow_function" | "closure_expression" | "func_literal"
                | "lambda" | "function_expression" | "async_function_expression"
            );
            if is_closure { AbstractKind::Closure } else { AbstractKind::FunctionDef }
        }
        ...
        NodeRole::Literal    => {
            ...
            match ts_kind {
                ...
                | "template_string" | "interpreted_string_literal"
                | "string_fragment" | "char_literal" => AbstractKind::StringLiteral,

                "number" | "integer_literal" | "float_literal"
                | "int_literal" | "number_literal" => AbstractKind::NumberLiteral,
