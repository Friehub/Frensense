    ...
    pub fn matches(&self, func_node: Node<'_>, source: &str) -> bool {
        ...
        if !self.contains_call_to.is_empty() {
            let has_match = self.contains_call_to.iter().any(|target| {
                calls.iter().any(|call| call.contains(target.as_str()))
            });
            if !has_match {
                return false;
            }
        ...
        if !self.contains_node_type.is_empty() {
            ...
            let has_match = self.contains_node_type.iter().any(|nt| {
        ...
        if !self.must_not_contain_node_type.is_empty() {
            let node_types = collect_node_types(func_node);
            let has_forbidden = self.must_not_contain_node_type.iter().any(|nt| {
                node_types.iter().any(|t| t == nt)
            });
            if has_forbidden {
                return false;
...
fn collect_call_targets(node: Node<'_>, source: &str) -> Vec<String> {
    ...
    loop {
        ...
        if n.kind() == "call_expression" {
            if let Some(callee) = n.child_by_field_name("callee") {
...
mod tests {
    ...
    fn test_function_name_regex() {
        ...
        assert!(filter.matches(func, "function sanitizeHtml(input: string) { return input; }"));

        let filter2 = SemanticFilter {
            function_name_regex: Some("^escape".to_string()),
            ..Default::default()
        };
        assert!(!filter2.matches(func, "function sanitizeHtml(input: string) { return input; }"));
    }
    ...
    fn test_contains_call_to() {
        ...
        assert!(filter.matches(func, "function foo() { fetch('/api').then(r => r.json()); }"));

        let filter2 = SemanticFilter {
            contains_call_to: vec!["axios".to_string()],
            ..Default::default()
        };
        assert!(!filter2.matches(func, "function foo() { fetch('/api').then(r => r.json()); }"));
    }
    ...
    fn test_must_not_contain_call_to() {
        ...
        let tree2 = parse_ts("function foo() { fetch('/api').then(r => r.json()).catch(e => {}); }");
        let func2 = tree2.root_node().child(0).unwrap();
        assert!(!filter.matches(func2, "function foo() { fetch('/api').then(r => r.json()).catch(e => {}); }"));
    }
}
