...
pub fn load_semantic_filters() -> std::collections::HashMap<String, SemanticFilter> {
    ...
    let sink_patterns: Vec<(&str, Vec<&str>)> = vec![
        ...
        ("ts_nextjs_metadata_og_image_ssrf", vec!["fetch", "request", "image"]),
        ("ts_remix_link_injection", vec!["Link", "navigate", "to"]),
        ("ts_graphql_batch_attack", vec!["graphql", "query", "batch", "operation"]),
        ("ts_vue_template_injection", vec!["template", "render", "compile"]),
        ("ts_svelte_html_injection", vec!["html", "template", "render"]),
        ("ts_bff_no_session_validation", vec!["session", "token", "bff"]),
        ("ts_oauth_overpermissive_scope", vec!["scope", "oauth", "authorize"]),
        ("ts_express5_redirect_order_leak", vec!["redirect", "status"]),
        ("ts_hardcoded_secret", vec!["secret", "key", "password", "token"]),
        ("ts_api_client_error_not_handled", vec!["fetch", "request", "axios", "catch", "then"]),
        ("tsx_a11y_onclick_not_keyboard", vec!["onClick", "onkeydown", "keyboard"]),
        ("tsx_tailwind_peer_validation_bypass", vec!["validate", "check", "submit"]),
        ("ts_type_confusion_string_number", vec!["parseInt", "Number", "toString", "String"]),
        ("ts_zod_promise_parse", vec!["parse", "safeParse", "validate"]),
        ("ts_race_condition_read_check_write_m4", vec!["transaction", "lock", "atomic", "increment"]),
        ("tsx_dangerously_set_inner_html_untrusted", vec!["dangerouslySetInnerHTML", "innerHTML"]),
        ("tsx_headlessui_dialog_focus_bypass", vec!["focus", "dialog", "modal"]),
        ("tsx_suspense_list_coalesce", vec!["Suspense", "SuspenseList", "revealOrder"]),
        ("tsx_radix_popover_side_conflict", vec!["Popover", "popover", "side"]),
        ("tsx_usememo_side_effect", vec!["useMemo", "useEffect"]),
        ("ts_node_cluster_shared_state", vec!["cluster", "worker", "send", "message"]),
        ("ts_ssrf_headless_browser", vec!["puppeteer", "playwright", "browser", "page"]),
        ("ts_prisma_raw_query_parameter_overflow", vec!["$queryRaw", "executeRaw", "raw"]),
        ("ts_crypto_weak_hash_sha1", vec!["sha1", "sha-1", "hash"]),
        ("tsx_astro_script_injection", vec!["script", "Astro"]),
        ("tsx_memo_comparison_bypass", vec!["memo", "useMemo", "compare"]),
        ("ts_booking_double_book", vec!["booking", "reserve", "lock", "transaction"]),
        ("ts_idor_share_no_check", vec!["share", "invite", "permission"]),
        ("ts_path_traversal_readfile_m4", vec!["readFile", "writeFile"]),
        ("ts_env_var_not_validated_startup", vec!["env", "process.env", "getenv"]),
        ("ts_mongoose_query_selector_injection", vec!["find", "findOne", "where", "regex"]),
        ("ts_timezone_scheduling", vec!["timezone", "utc", "date", "temporal"]),
        ("ts_rbac_missing_on_update", vec!["role", "permission", "auth", "access"]),
        ("ts_order_modification_after_fulfil", vec!["order", "status", "update", "fulfill"]),
        ("ts_dataflow_mixed_return_types", vec!["Error", "Result", "throw", "return"]),
        ("ts_s3_presigned_url_no_expiry", vec!["s3", "presigned", "getSignedUrl"]),
        ("tsx_rn_linking_open_url", vec!["Linking", "openURL", "openUrl"]),
        ("ts_mfa_code_reuse", vec!["otp", "totp", "mfa", "verify"]),
        ("ts_promise_fire_forget_side_effect", vec![".then", "await", "promise"]),
        ("ts_bac_feature_flag_bypass", vec!["flag", "feature", "toggle"]),
        ("ts_electron_context_isolation_off", vec!["contextIsolation", "nodeIntegration"]),
        ("ts_eval_vm_script", vec!["vm", "runInContext", "Script"]),
        ("ts_angular_bypasssecuritytrust", vec!["bypassSecurityTrust", "sanitizer"]),
        ("ts_temporal_instant_from_epoch_missing", vec!["Temporal", "Instant", "epoch"]),
        ("ts_buffer_slice_dependent", vec!["slice", "buffer", "Buffer"]),
        ("tsx_server_action_missing_auth_check", vec!["server", "action", "auth"]),
        ("ts_tenant_cross_tenant_data", vec!["tenant", "organization", "where"]),
        ("ts_missing_ledger_after_wallet_fund", vec!["ledger", "wallet", "fund", "credit"]),
        ("tsx_error_boundary_render_crash_m2", vec!["error", "Error", "fallback"]),
        ("tsx_ref_forwarding_untrusted", vec!["forwardRef", "ref", "innerHTML"]),
        ("ts_nestjs_circular_dependency", vec!["Module", "import", "forwardRef"]),
        ("tsx_error_boundary_render_crash_m4", vec!["error", "Error", "boundary"]),
