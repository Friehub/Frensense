...
impl Default for PerCategoryCalibration {
    fn default() -> Self {
        Self {
            global: CalibrationParams::default(),
...
impl PerCategoryCalibration {
    ...
    pub fn train(scores_and_labels: &[(String, Vec<f64>, Vec<bool>)]) -> Self {
        ...
        let all_scores: Vec<f64> = scores_and_labels.iter()
            .flat_map(|(_, scores, _)| scores.iter().cloned())
            .collect();
        let all_labels: Vec<bool> = scores_and_labels.iter()
        ...
        for (category, scores, labels) in scores_and_labels {
            if scores.len() >= 20 { // Minimum samples for reliable calibration
...
pub fn save_per_category_calibration(cal: &PerCategoryCalibration, path: &std::path::Path) -> Result<(), String> {
...
mod tests {
    ...
    fn test_per_category_calibration() {
        ...
        cal.categories.insert("cmd_injection".to_string(), CalibrationParams {
            a: 4.0,
            b: -1.5,
            n_samples: 100,
            accuracy: 0.85,
    ...
    fn test_train_per_category() {
        ...
        let data = vec![
            ("cmd_injection".to_string(), scores_injection, labels_injection),
