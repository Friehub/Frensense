...
pub fn find(snap: &FileSnapshot, _ctx: &FindingContext<'_>) -> Vec<Advisory> {
    ...
    if fname.ends_with("user-dao.js")
        || fname.ends_with("user_dao.js")
        || fname.ends_with("profile-dao.js")
        || fname.ends_with("profile_dao.js")
    {
        ...
        if active.contains("password") && !has_active_password_hash {
            ...
            if let Some(line) = line {
                if active.contains(".save(")
                    || active.contains(".insert(")
                    || active.contains(".update(")
                    || active.contains("dao.")
                    || active.contains("password //")
                    || active.contains("password,")
                {
                    advisories.push(
        ...
        for line in find_active_lines(source, |l| {
            ...
            if l.contains("===")
                && (l.contains("fromDB")
                    || l.contains("fromUser")
                    || l.contains("comparePassword")
                    || l.contains("compare_password"))
            {
                return true;
            }
            false
    ...
    if fname.ends_with("profile-dao.js")
        || fname.ends_with("profile_dao.js")
        || fname.contains("profile")
    {
        let active = strip_comments(source);
        if !active.contains("encrypt") && !active.contains("cipher") && !active.contains("aes") {
            let ssn_lines: Vec<u32> = source
                .lines()
                .enumerate()
                .filter(|(_, l)| {
                    let t = l.trim();
    ...
    if fname.ends_with("-dao.js") || fname.ends_with("_dao.js") {
        for line in find_active_lines(source, |l| l.contains("$where") && l.contains("${")) {
            advisories.push(
    ...
    if source.contains("Math.random")
        ...
        && !source.contains("uuid")
        && !source.contains("randomBytes")
    {
...
fn find_active_line(source: &str, pattern: &str) -> Option<u32> {
    find_active_lines(source, |l| l.contains(pattern))
        .into_iter()
        .next()
}
