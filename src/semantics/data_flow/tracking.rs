    ...
    pub fn analyze_block(&self, node: Node<'a>, registry: &mut TaintRegistry) -> Vec<Advisory> {
        ...
        for op in self.context.semantic_ops {
            let op_range = match op {
                SemanticOp::Binding { value_range, .. } => *value_range,
                SemanticOp::Assignment { value_range, .. } => *value_range,
                SemanticOp::Call { range, .. } => *range,
                SemanticOp::EnterBlock(range) => *range,
            };
                    ...
                    && use_entry.end_byte <= op_range.end_byte
                {
                    ...
                    for def in reaching {
                        if let Some(taint_origin) = registry.get_origin(&def.name) {
                            registry.taint(&use_entry.name, taint_origin);
                        }
                    }
                }
            }
