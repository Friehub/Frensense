...
use super::{extract_by_patterns, AuthConvention, FrameworkAdapter};
...
impl FrameworkAdapter for RemixAdapter {
    ...
    fn extract_injection_points(&self, body: &str) -> Vec<InjectionPoint> {
        ...
        if body.contains("request.json()") { points.push(super::body_point()); }
        if body.contains("request.formData()") { points.push(super::form_point()); }
        points
    }
