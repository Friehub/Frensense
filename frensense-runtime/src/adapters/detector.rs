...
use super::{FrameworkAdapter, UnknownAdapter, express::ExpressAdapter};
...
fn detect_ts_framework(root: &Path) -> Box<dyn FrameworkAdapter> {
    ...
    if has_dep(&pkg, "next") {
        return Box::new(super::nextjs::NextJsAdapter);
    }
    if has_dep(&pkg, "@remix-run/node") || has_dep(&pkg, "@remix-run/react") {
        return Box::new(super::remix::RemixAdapter);
    }
    if has_dep(&pkg, "@sveltejs/kit") {
        return Box::new(super::sveltekit::SvelteKitAdapter);
    }
    if has_dep(&pkg, "astro") {
        return Box::new(ExpressAdapter);
    }
    if has_dep(&pkg, "@nestjs/core") {
        return Box::new(super::nestjs::NestJsAdapter);
    }
    if has_dep(&pkg, "fastify") {
        return Box::new(super::express::FastifyAdapter);
    }
    if has_dep(&pkg, "hono") {
        return Box::new(super::hono::HonoAdapter);
    }
    if has_dep(&pkg, "@trpc/server") {
        return Box::new(ExpressAdapter);
    }
    if has_dep(&pkg, "express") {
        return Box::new(ExpressAdapter);
    }
